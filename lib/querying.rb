...
def select_series_title_with_most_human_characters
   "SELECT series.title
  FROM characters
  JOIN series ON(characters.series_id = series.id) GROUP BY characters.species = 'human' LIMIT 1 ;"
end
...
def select_character_names_and_number_of_books_they_are_in
  ...
  INNER JOIN books ON(books.id = character_books.book_id) GROUP BY characters.name ORDER BY COUNT(books.id) DESC;"
end
