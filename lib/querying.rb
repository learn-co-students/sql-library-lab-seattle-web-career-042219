...
def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(books.id) FROM characters INNER JOIN character_books on characters.id = character_books.character_id
  INNER JOIN books on character_books.book_id = books.id GROUP BY characters.name ORDER BY COUNT(books.id) DESC;"
end
